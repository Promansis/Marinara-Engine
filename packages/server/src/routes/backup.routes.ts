...
import { ltmSafeRelativePathSchema, type ExportEnvelope } from "@marinara-engine/shared";
...
function stageLongTermMemoryArchive(input: ProfileImportInput) {
  ...
  const hasCanonicalData = entries.some((entry) => {
    ...
    return ["vault/", "config/", "events/", "drafts/", "transactions/"].some((directory) => path.startsWith(directory));
  });
...
function buildBackupRestoreNotes() {
  return [
    ...
    "Manual directory recovery remains available for older backups that do not contain a Marinara profile archive.",
    "",
...
export async function backupRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (req, reply) => {
    ...
    try {
      ...
      // 2. Copy data directories. LTM is copied under its vault lock so this
      // filesystem backup represents one coherent canonical snapshot.
  ...
  app.post("/download", async (req, reply) => {
    ...
    try {
      ...
      // Stage LTM first so zipping does not interleave with a vault mutation.
