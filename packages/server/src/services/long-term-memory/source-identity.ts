...
import type { LtmNote, LtmSourceProvenance } from "@marinara-engine/shared";
...
function evidenceValue(evidence: readonly string[], prefix: string) {
  return evidence.find((entry) => entry.startsWith(prefix))?.slice(prefix.length).trim();
}
...
export function inferSourceProvenance(note: Pick<LtmNote, "tags" | "sections" | "provenance">) {
  ...
  if (note.tags.includes("imported_chat")) {
    ...
    return entryId
      ? ({ kind: "chat_summary", sourceId, entryId } satisfies LtmSourceProvenance)
      : null;
  }
  return null;
}
