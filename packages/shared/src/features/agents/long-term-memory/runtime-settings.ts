...
export function resolveLongTermMemoryRecallSettings(input: {
  ...
  return {
    enabled: readBoolean(chatMetadata.enableLongTermMemory) ?? globalSettings?.enableLongTermMemory ?? false,
    budgetTokens:
    ...
    recallStyle,
    weights: resolveWeights(chatMetadata, chatRecallStyle ? styleWeights : globalWeights),
    debugEnabled:
